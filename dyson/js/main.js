(function () {
    ...
    //     // Скрывать бейдж, если товаров 0 (опционально)
    ...
    const mySwiper = new Swiper(".mySwiper", {
        slidesPerView: 3,        // 3 колонки на десктопе
        grid: { rows: 2 },       // 2 ряда
        slidesPerGroup: 6,       // Перелистывать по 6 штук (пачкой)
        spaceBetween: 20,
        navigation: {
        ...
        breakpoints: {
            // На мобильных (до 768px) перестраиваем в 2 колонки
            320: {
                slidesPerView: 2,
                grid: { rows: 2 },
                slidesPerGroup: 4,   // Перелистываем по 4 на мобильных
            },
            768: {
                slidesPerView: 3,
                grid: { rows: 2 },
    ...
    document.querySelectorAll('.product-card').forEach(card => {
        ...
        buyBtn.addEventListener('click', () => {
            if (currentCount > 0) {
                console.log(`Добавлено в корзину: ${currentCount} шт.`);
                // Здесь можно вызвать функцию добавления в корзину
            } else {
